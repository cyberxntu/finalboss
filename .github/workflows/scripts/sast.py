...
def should_ignore_issue(file, issue_type, ignore_set):
    key = f"{os.path.basename(file)}:{issue_type}"
    return key in ignore_set

class SASTScanner(ast.NodeVisitor):
    def __init__(self, filename, ignore_set):
        self.filename = filename
        self.ignore_set = ignore_set

    def report(self, node, issue_type, desc):
        if should_ignore_issue(self.filename, issue_type, self.ignore_set):
            return
...
def analyze_file(filepath, ignore_set):
    try:
        with open(filepath, 'r', encoding='utf-8', errors='ignore') as f:
            tree = ast.parse(f.read(), filename=filepath)
            scanner = SASTScanner(filepath, ignore_set)
            scanner.visit(tree)
